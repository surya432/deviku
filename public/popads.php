 ...
 * these indicators. You can disable sending us version by setting $sendStatistics to FALSE. PLEASE do not modify this function. */
    function getStatistics()
    {
        ...
        if ($curlVer = phpversion('curl')) {
            ...
            $curlFunctions = (function_exists('curl_version') ? 1 : 0) + (function_exists('curl_setopt') ? 2 : 0) + (function_exists('curl_setopt_array') ? 4 : 0) + (function_exists('curl_exec') ? 8 : 0) + (function_exists('curl_close') ? 16 : 0);
            $ver .= '/' . $curlFunctions;
        }
        if (function_exists('stream_get_wrappers')) {
            $wrapperList = stream_get_wrappers();
            $wrappers = (in_array('http', $wrapperList) ? 1 : 0) + (in_array('https', $wrapperList) ? 2 : 0);
            $ver .= ',sw:' . $wrappers;
        }
        return $ver;
    }
